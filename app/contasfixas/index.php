	...
	<li><a href="#" onClick ="$('#table-1').tableExport({type:'pdf',pdfFontSize:'7',escape:'false'});"> <img src='assets/images/pdf.png' width='24px'> PDF</a></li>
	      ...
	      <br>
 		  ...
 		  $consultas = $mysqli->query("SELECT * FROM lc_fixas WHERE empresa = '$idempresa'");
 		  while($campo = mysqli_fetch_array($consultas)){
		  
		  ?>
		  <tr>
                     <td><?php echo $campo['id']; ?></td>
                      <td><?php echo $campo['descricao_fixa']; ?></td>

                       <td>
		      <?php
		      $ids = $campo['cat'];
		      $ccat = $mysqli->query("SELECT * FROM lc_cat WHERE id = '$ids'");
 		      $ccategoria = mysqli_fetch_array($ccat);
 		      ?><?php echo $ccategoria['nome']; ?></td>

                      <td><?php echo $campo['dia_vencimento']; ?></td>
                      <td>R$ <?php echo number_format($campo['valor_fixa'], 2, ',','.'); ?></td>

                      <td>
	      <a href="?app=CadastroContasFixas&id=<?php echo base64_encode($campo['id']); ?>" class="btn btn-info tooltiped" data-toggle="tooltip" data-placement="top" title="Alterar"><i class="entypo-tools"></i></a>&nbsp;
	      <?php if($logado['nivel'] == '1') { ?> 
	      	 <a href="javascript:void(0);" onclick="javascript: if (confirm('Deseja realmente excluir esse registro ?')) { window.location.href='?app=CadastroContasFixas&id=<?php echo base64_encode($campo['id']); ?>&Ex=Del' } else { void('') };"  class="btn btn-danger tooltiped" data-toggle="tooltip" data-placement="top" title="Excluir"><i class="entypo-trash"></i></a>
	      <?php } ?>
		      </td>
                    </tr>
