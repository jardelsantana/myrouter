...
?>
	...
	<div class="alert alert-info alert-dismissable">
        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">
	<i class="fa fa-times-circle"></i></button>
        <strong>Atenção!</strong> Backup do Mikrotik realizado com sucesso. </div>
	<?php } ?>
	...
	<?php } ?>
        ...
        <div class="page-header">
          <h1>Servidores<small>Mikrotiks</small></h1>
        </div>
        
        <a href="?app=CadastroServidor" class="btn btn-info">NOVO SERVIDOR</a>
        <br><br>
        ...
        <div class="row" id="powerwidgets">
          <div class="col-md-12 bootstrap-grid"> 
            ...
            <div class="powerwidget" id="" data-widget-editbutton="false">
              <header>
                <h2>Gerenciar<small>Mikrotiks</small></h2>
              </header>
	...
	<li><a href="#" onClick ="$('#table-1').tableExport({type:'pdf',pdfFontSize:'7',escape:'false'});"> <img src='assets/images/pdf.png' width='24px'> PDF</a></li>
		  ...
		  ini_set("track_errors","0");
 		  ...
 		  $consultas = $mysqli->query("SELECT * FROM servidores WHERE tiporouter = 'MIKROTIK' AND empresa = '$idempresa'");
	...
	<?php if($campo['tipo'] == '24') { ?><img style="height:60px;" src="assets/images/antenas/24.png"><?php } ?>
		      ...
		      $(document).ready(function() {
 	 	      $("#up<?php echo $campo['id']; ?>").load("app/uptime.php?ip=<?php echo base64_encode($campo['ip']); ?>&login=<?php echo base64_encode($campo['login']); ?>&senha=<?php echo base64_encode($campo['senha']); ?>");
	       ...
	       <a href="?app=CadastroServidor&id=<?php echo base64_encode($campo['id']); ?>" class="btn btn-info tooltiped" data-toggle="tooltip" data-placement="top" title="Configurar"><i class="entypo-tools"></i></a>&nbsp;

           <a href="javascript:void(0);" onclick="javascript: if (confirm('Deseja realmente excluir esse registro ?')) { window.location.href='?app=CadastroServidor&id=<?php echo base64_encode($campo['id']); ?>&Ex=Del&nome=<?php echo $campo['nome']; ?>&srv=<?php echo $campo['servidor']; ?>' } else { void('') };" class="btn btn-danger tooltiped" data-toggle="tooltip" data-placement="top" title="Excluir"><i class="entypo-trash"></i></a>
